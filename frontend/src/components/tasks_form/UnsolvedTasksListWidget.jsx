...
const UnsolvedTasksListWidget = ({ compact = false }) => {
    ...
    useEffect(() => {
        const fetchUnsolvedTasks = async () => {
            try {
                const response = await axios.get(`http://127.0.0.1:8000/tasks/get_unsolved_tasks?page=${currentPage}&limit=${tasksPerPage}`);
                setTasks(response.data.tasks);
                setTotalPages(response.data.total_pages);
            } catch (error) {
                console.error('Ошибка при получении нерешённых заданий:', error);
            }
        };
    ...
    return (
        ...
        >
            {tasks.length === 0 ? (
                <p className="text-center text-white text-lg mt-10">Нерешённых заданий пока нет.</p>
            ) : (
                <>
...
export default UnsolvedTasksListWidget;
